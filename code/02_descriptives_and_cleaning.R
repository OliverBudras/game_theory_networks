...
data <- map_dfr(files, read_data,.progress = list(
  type = "iterator", 
  format = "Loading {cli::pb_bar} {cli::pb_percent}",
  clear = TRUE)) |> 
...
data_clean <- data |> 
  ...
  filter(publication_year > 1949,publication_year < 2025)
...
data_clean2 <- data_clean |> 
  filter(publication_year  >= 1980 & publication_year < 2025) 
...
### Function for generating descriptives (tables and figures)
...
descriptives <- function(data){
  ...
  articles_per_domain_plt <- ggplot(articles_per_domain,aes(No_publications, fct_reorder(domain_1, No_publications))) +
    geom_bar(stat = "identity") +
  ...
  articles_per_domain_plt2 <- ggplot(articles_per_domain,aes(No_citations, fct_reorder(domain_1, No_citations))) +
    geom_bar(stat = "identity") +
  ...
  query_dist <- data |> 
    ...
    ggplot(aes(x = query, y = N, fill = field_1)) +
...
descriptives_list <- descriptives(data_final_1980)
...
descriptives_list$concept_dist |> plot()
descriptives_list$query_dist |> plot()
descriptives_list$wc |> replayPlot()

saveRDS(descriptives_list, "v3/descriptives.RDS")
saveRDS(data_final_1980, "v3/data_final_1980.RDS")
