...
<div class="field"><span class="label">Cluster:</span><span class="value">${n.id ?? ""}</span></div>
<div class="field"><span class="label">Top Words:</span><span class="value">${n.label_full ?? ""}</span></div>
...
const firstJsonIndex = 1; // nodes_window_001.json

const fileIndex = windowIndex + firstJsonIndex;

const nodes = await fetch(`public/windows_field_aggregate_cluster/nodes_window_${String(fileIndex).padStart(3,"0")}.json`)
  .then(r => r.json());

const edges = await fetch(`public/windows_field_aggregate_cluster/edges_window_${String(fileIndex).padStart(3,"0")}.json`)
  .then(r => r.json());
